...
class _DriverHomeTabState extends State<DriverHomeTab>
  ...
  Future<void> _acceptRequest(String requestId) async {
    try {
      ...
      // Fetch fresh row (already has drop_lat/lng/location set by customer)
      ...
      final freshMap = Map<String, dynamic>.from(fresh as Map);

      // Inject customer phone
      final customerId = freshMap['customer_id'] as String?;
      if (customerId != null) {
        try {
          final profile = await _sb
              ...
              .eq('id', customerId)
              .maybeSingle();
          freshMap['customer_phone'] = profile?['phone'] as String?;
        } catch (_) {
          freshMap['customer_phone'] = _pendingRequest?['customer_phone'];
        }
      }

      // Clear pending and go straight to live map — NO hospital picker
      if (mounted) {
        setState(() {
          _pendingRequest = null;
          _loadingRequest = false;
        });
        widget.onRequestAccepted(freshMap);
      }
    } catch (e) {
...
class _DriverRideFlowState extends State<DriverRideFlow>
  ...
  void _startRealGpsTracking() {
    ...
    // Update driver position from real GPS every 5 seconds
    _gpsSubscription = Geolocator.getPositionStream(
      ...
      // Write to DB so customer can see real driver location
      void _startRealGpsTracking() {
        _gpsSubscription?.cancel();
        // Update driver position from real GPS every 5 seconds
        _gpsSubscription = Geolocator.getPositionStream(
          ...
          // Write to DB so customer can see real driver location
          _writeDriverLocationToDB(pos.latitude, pos.longitude);
        });
      }
    });
  }

  Future<void> _writeDriverLocationToDB(double lat, double lng) async {
    ...
    try {
      ...
      debugPrint('[GPS] DB write error: $e');
    }
  }
...
// Call this periodically in _startDriverAnimation or with Timer
  ...
  Future<void> _initPositions() async {
    ...
    // FIX: Start REAL GPS tracking, not fake animation
  ...
  double _distanceKm(double lat1, double lng1, double lat2, double lng2) {
    ...
    final a = sin(dLat / 2) * sin(dLat / 2) + // FIX: add +
        cos(_deg2rad(lat1)) *
            cos(_deg2rad(lat2)) * // now part of formula
            sin(dLng / 2) *
            sin(dLng / 2);
  ...
  // Map each RidePhase → DB status string.
  // Values MUST match the CHECK constraint on ambulance_requests.status:
  // ('waiting','accepted','enRoute','arrivedAtScene','enRouteToHospital','completed','cancelled')
  static const _phaseDbStatus = {
    RidePhase.enRoute: 'enRoute',
    RidePhase.arrivedAtScene: 'arrivedAtScene',
    RidePhase.enRouteToHospital: 'enRouteToHospital',
    RidePhase.delivered: 'completed',
  };
  ...
  Future<void> _advance() async {
    ...
    final requestId = widget.activeRequest?['id']?.toString();
    final dbStatus = _phaseDbStatus[nextPhase];

    // FIX: write status for EVERY phase, not only 'completed'
    if (requestId != null && dbStatus != null) {
      try {
        final payload = <String, dynamic>{'status': dbStatus};
        ...
        await Supabase.instance.client
            ...
            .update(payload)
            .eq('id', requestId);
        debugPrint('[_advance] wrote status=$dbStatus for $requestId');
      } catch (e) {
  ...
  void didUpdateWidget(DriverRideFlow old) {
    ...
    if (old.activeRequest != null && widget.activeRequest == null) {
      ...
      _moveTimer?.cancel();
      _gpsSubscription?.cancel(); // ADD THIS
    }
              ...
              Semantics(
                ...
                child: InkWell(
                  ...
                  onTap: () async {
                    ...
                    debugPrint('[CALL] customer_phone = $phone');
                    ...
                    try {
                      if (await canLaunchUrl(uri)) {
                        await launchUrl(
                          uri,
                          mode: LaunchMode
                              .externalApplication, // FIX: forces phone app
                        );
                      } else {
                        if (mounted) {
                          showErrorSnack(context, 'Cannot open phone app');
                        }
                      }
                    } catch (e) {
                      debugPrint('[CALL] error: $e');
                      if (mounted) {
                        showErrorSnack(context, 'Call failed: $e');
                      }
                    }
                  },
                  child: Container(
...
class _DriverFullscreenMap extends StatelessWidget {
  ...
  // Opens Google Maps with real navigation
  Future<void> _navigateWithMaps(BuildContext context) async {
    final toPickup = phase.index <= RidePhase.enRoute.index;
    final destLat = toPickup ? pickupLat : (dropLat ?? pickupLat);
    final destLng = toPickup ? pickupLng : (dropLng ?? pickupLng);

    // Google Maps navigation URL — works on Android and iOS
    // Opens Google Maps app if installed, browser otherwise
    ...
    try {
      ...
      if (context.mounted) {
        showErrorSnack(context, 'Could not open maps: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final toPickup = phase.index <= RidePhase.enRoute.index;
    final destLabel = toPickup ? 'Navigate to Pickup' : 'Navigate to Hospital';
    final navColor = toPickup ? AppColors.red : const Color(0xFF2196F3);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // ── Navigate button at bottom — like donor module ──────────────
          Positioned(
            bottom: 32,
            left: 24,
            right: 24,
            child: ElevatedButton.icon(
              ...
              onPressed: () => _navigateWithMaps(context),
